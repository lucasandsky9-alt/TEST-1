...
# Only enable the intents we actually use. Presences and typing events in
# particular are expensive to keep in memory and this bot never reads them.
...
class MyBot(commands.Bot):
    def __init__(self):
        super().__init__(
            ...
            intents=intents,
            help_command=None,
            # Don't cache recent messages — nothing in this bot reads message
            # history from the cache (purge/rawmessage fetch directly from
            # the API), so this avoids holding thousands of Message objects.
            max_messages=None,
            # Don't eagerly download and cache every member of every guild
            # on startup — that's the single biggest memory cost on a bot
            # in any reasonably sized server.
    ...
    async def on_ready(self):
        ...
        # Keep the account's Discord username in sync with BOT_NAME. This is
        # rate-limited by Discord (roughly 2 changes/hour), so it's a no-op
        # once the name already matches.
        if self.user.name != BOT_NAME:
            try:
                await self.user.edit(username=BOT_NAME)
                log.info(f"Renamed bot account to '{BOT_NAME}'.")
            except discord.HTTPException as e:
                log.warning(f"Could not rename bot to '{BOT_NAME}': {e}")
